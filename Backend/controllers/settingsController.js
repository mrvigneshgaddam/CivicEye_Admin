...
exports.getSettings = async (req, res) => {
  try {
    const settings = await Settings.findOne({ policeId: req.user.id });
...
exports.saveSettings = async (req, res) => {
  try {
    ...
    let settings = await Settings.findOne({ policeId: req.user.id });
    
    if (!settings) {
      settings = new Settings({ policeId: req.user.id });
    }
  ...
  try {
    ...
    if (success) {
      ...
      if (settings.security.failedLoginAttempts >= maxAttempts) {
        ...
        if (police) {
          try {
            await sendAdminEmail(police);
            console.log(`Account lock email sent to: ${police.email}`);
          } catch (emailError) {
          ...
          if (process.env.ADMIN_EMAIL) {
            try {
              await sendAdminNotification(police);
              console.log(`Admin notification sent for: ${police.email}`);
...
exports.checkAccountLockMiddleware = async (req, res, next) => {
  try {
    const settings = await Settings.findOne({ policeId: req.user.id });
